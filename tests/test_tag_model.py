...
class TagModelTestCase(unittest.TestCase):
    ...
    def test_generate_tags(self):
        ...
        self.assertEqual(
            sorted([tag.name for tag in tags1]), sorted(tag_names1))
        self.assertEqual(
            sorted([tag.name for tag in tags2]), sorted(tag_names2))
        self.assertEqual(sorted([tag.name for tag in tags3]),
                         sorted(['tag1', 'tag2', 'tag3']))
